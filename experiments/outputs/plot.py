...
if __name__ == '__main__':
    ...
    for filename in args.file:
        ...
        steps = main_df.groupby('step_time').queue_sum.mean().keys().values
        # mean = moving_average(main_df.groupby('step_time').mean()['step_wait_time'], window_size=args.window)
        mean = moving_average(main_df.groupby('step_time').mean()['queue_sum'], window_size=args.window)
        #sem = moving_average(main_df.groupby('step_time').sem()['total_wait_time'], window_size=args.window)
        std = moving_average(main_df.groupby('step_time').std()['queue_sum'], window_size=args.window)
