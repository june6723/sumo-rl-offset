...
class TrafficSignal:
    ...
    def is_learnable(self):
        # if current phase is yellow or time_on_phase doesn't excess min green time 
        if self.phase % 2 == 1 or self.min_green[self.phase//2] > self.time_on_phase : 
            return 0
        else :
            return 1
    ...
    def set_next_phase(self, action):
        ...
        "Version2 - You can simulate env in 1 delta second"
        if self.phase % 2 == 0:
            self.current_min_green = self.min_green[self.phase//2]
            if action == 0 or self.time_on_phase < self.current_min_green:
            ...
            if self.phase == (self.num_green_phases*2-1):  # last yellow phase
                if self.time_on_phase < self.env.yellow_time:
                    self.time_on_phase += 1
                    traci.trafficlight.setPhase(self.id, self.phase) 
                else:
                    self.time_on_phase = 1
                    traci.trafficlight.setPhase(self.id, 0)  # First green phase
                ...
                if self.time_on_phase < self.env.yellow_time:
                    ...
                    self.time_on_phase = 1
                    traci.trafficlight.setPhase(self.id, self.phase + 1)  # turns green
